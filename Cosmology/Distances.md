...
The comoving coordinate system evolves "with" the Hubble flow and is related to the [[Units#Proper distance|proper distance]] $d_p = a(t)\,d_C$ at a particular time $t$ where $a$ is the scale factor of the [[FLRW#Friedmann equation|Friedmann equations]]. It is essentially the $r$ coordinate in the [[FLRW#FLRW metric|FLRW metric]] and is given by $$d_C(z) =  \frac{c}{H_0} \int_0^{z} \frac{dz}{E(z)}$$where we have defined $$E(z) = \frac{H(z)}{H_0} = \sqrt{\Omega_{r}(1+z)^4 + \Omega_M(1+z)^3 + \Omega_k(1+z)^2 + \Omega_\Lambda}$$
**Derivation**
...
An object of size $x$ at redshift $z$ that appears to have angular size $\delta\theta$ has the angular diameter distance of $d_A ( z ) = x / \delta\theta$. This is commonly used to observe so called "standard rulers", for example in the context of [[BAO#BAO|baryon acoustic oscillations]].
