...
Lets quantify the total number of photons observed $N$ and the various contributions to noise in those counts, without making explicit any frequency dependence.
...
The total magnitude of the noise is then the following, assuming all the above are independent $$\sigma = \sqrt{\sigma_s^2+\sigma_b^2+\sigma_d^2+\sigma_r^2} = \sqrt{N_s+N_b+N_d+N_r^2}$$Thus the signal to noise ratio is $$\frac{\rm{Signal}}{\rm{Noise}}= \frac{N_s}{\sigma}$$**What terms dominate at optical versus infrared wavelengths?**
...
At IR wavelengths the dark current is significant so $\sigma \simeq \sqrt{N_d}$ (why? not sure, but may be due to thermal motion of particles within material of telescope itself, hence cooling of [[Observatories#JWST|JWST]] for example).

**Why, when observing objects that are fainter than physical foregrounds like the night sky, does SNR increase as sqrt(time)?**

Look at the observation in terms of intrinsic rates multiplied by exposure times $$N_s = R_st,\quad N_b =nR_bt,\quad N_d = nR_dt, \quad N_r = nR_r $$where $n$ is the number of pixels, $R_s$ has units photons per second and the others have units photons per second per pixel (this isnt really necessary for this question, could deal with totals), noting that the read-out is time-independent. We can rewrite the signal to noise as $$\frac{\rm{Signal}}{\rm{Noise}}= \frac{R_s t}{\sqrt{R_s t + n(R_b t + R_d t + R_r^2)}} = \frac{R_s \sqrt{t}}{\sqrt{R_s + n(R_b + R_d + R_r^2/t)}}$$So for everything except readout noise-dominated, it scales as $\sqrt{t}$. 
...
Examples: Hipparcos, [[Observatories#Kepler|Kepler]]
![[schmidt.png|400]]
...
- integral field unit (IFU, e.g [[Observatories#Gemini|Gemini]])
...
Moon acts roughly like a coronograph during solar eclipse
...
Deformable mirrors for ground-based telescopes which can counter atmospheric turbulence and blurring using bright sources on the sky as "guides" for very fast timescale corrections. Measure time-dependent deflection of bright source and correct that deflection. We sometimes shoot lasers into the atmosphere to create small, well-understood false "stars" to use for corrections instead.
...
- GW170817 (17th Aug 2017) a binary [[Neutron stars#Neutron stars|NS]] merger followed up by [[Observatories#Fermi|Fermi]] [[GRBs#GRB|GRB]] detection.
