...
The problem is we usually dont know when all the light was emitted, so we cant measure a single time delay. Instead we use the differences in arrival times $\Delta t$ between two frequencies, and use that to determine the electron density $n_e$ (or really the column density).
